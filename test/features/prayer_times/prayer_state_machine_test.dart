...
void main() {
  ...
  group('PrayerStateMachine.advance', () {
    test('keeps upcoming prayer before scheduled time', () {
      expect(
        machine
            .advance(entry(), scheduled.subtract(const Duration(seconds: 1)))
            .status,
    ...
    test('moves prayer to pending when grace period expires', () {
      expect(
        machine
            .advance(entry(), scheduled.add(const Duration(hours: 1)))
            .status,
    ...
    test('returns snoozed prayer to pending after snooze expires', () {
      final DateTime until = scheduled.add(
        const Duration(hours: 1, minutes: 20),
      );
      final PrayerEntry snoozed = entry(
    ...
    test('does not rewrite final state', () {
      final PrayerEntry prayed = machine.confirm(
        entry(),
        scheduled.add(const Duration(minutes: 10)),
      );
      expect(
        machine.advance(prayed, scheduled.add(const Duration(days: 1))).status,
        PrayerStatus.prayed,
      );
    });
  });
  ...
  group('actions', () {
    test('confirmation stores time and prayed state', () {
      ...
      final PrayerEntry result = machine.confirm(
    ...
    test('snooze increments counter and schedules next pending time', () {
      ...
      final PrayerEntry result = machine.snooze(
        entry(status: PrayerStatus.pending),
        now,
    ...
    test('skip produces skipped final state', () {
      expect(
        machine.skip(entry(status: PrayerStatus.pending)).status,
