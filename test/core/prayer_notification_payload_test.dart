...
void main() {
  ...
  test(
    'versioned payload round trips prayer, action, and delivery identity',
    () {
      const payload = PrayerNotificationPayload(
        prayerId: id,
        kind: 'reminder',
        action: PrayerNotificationAction.snooze,
        eventId: '42:123456789',
      );
  ...
  test(
    ...
    () {
      ...
      final mark = PrayerNotificationPayload.fromResponse(
        payload.encode(),
        'mark_prayed',
      )!;
      expect(mark.action, PrayerNotificationAction.markPrayed);
      expect(mark.eventId, 'scheduled');
      final snooze = PrayerNotificationPayload.fromResponse(
        'prayer:$id',
        'snooze',
        eventId: 'response',
      )!;
  ...
  test(
    'untrusted malformed, oversized, or unsupported payloads are ignored',
    () {
      for (final invalid in <String?>[
        ...
        expect(
          PrayerNotificationPayload.tryParse(invalid),
          isNull,
          reason: '$invalid',
        );
      }
